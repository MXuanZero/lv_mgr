...
{
	const lv_coord_t hres = disp_drv->physical_hor_res == -1 ? disp_drv->hor_res :
															   disp_drv->physical_hor_res;
	const lv_coord_t vres = disp_drv->physical_ver_res == -1 ? disp_drv->ver_res :
	...
	LV_COLOR_DEPTH != 32 /*32 is valid but support 24 for backward                            \
																				compatibility \
							too*/
	int32_t x;
	...
	while (SDL_PollEvent(&event)) {
		...
		if ((&event)->type == SDL_WINDOWEVENT) {
			...
			case SDL_WINDOWEVENT_MOVED:
				win_param_pos_set((&event)->window.data1, (&event)->window.data2);
				break;
			default:
				break;
			}
...
static void window_create(monitor_t *m)
{
	...
	int x, y;
	if (win_param_pos_get(&x, &y) == PARAM_OK) {
		m->window =
			SDL_CreateWindow("LVGL", x, y, SDL_HOR_RES * SDL_ZOOM, SDL_VER_RES * SDL_ZOOM, flag);
	} else {
		m->window = SDL_CreateWindow("LVGL", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
