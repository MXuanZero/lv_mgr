...
`lv_timer_ready(timer)` makes a timer run on the tail call of `lv_timer_handler()`.
...
For these cases, `lv_async_call(my_function, data_p)` can be used to call `my_function` on the tail invocation of `lv_timer_handler`. `data_p` will be passed to the function when it's called.
...
/*Delete screen on tail call of `lv_timer_handler`, not right now.*/
...
If you just want to delete an object and don't need to clean anything up in `my_screen_cleanup` you could just use `lv_obj_del_async` which will delete the object on the tail call to `lv_timer_handler`.
