...
- **LV_KEY_NEXT** Focus on the tail object
...
In *Navigate* mode, an encoder's `LV_KEY_LEFT/RIGHT` is translated to `LV_KEY_NEXT/PREV`. Therefore, the tail or previous object will be selected by turning the encoder.
