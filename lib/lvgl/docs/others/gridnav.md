...
To move the focus to the tail widget of the group use `LV_KEY_NEXT/PREV` or `lv_group_focus_next/head()` or the `TAB` key on keyboard as usual.
...
- `LV_GRIDNAV_CTRL_ROLLOVER`  If there is no tail/previous object in a direction,
the focus goes to the object in the tail/previous row (on left/right keys) or first/last row (on up/down keys
- `LV_GRIDNAV_CTRL_SCROLL_FIRST` If an arrow is pressed and the focused object can be scrolled in that direction
then it will be scrolled instead of going to the tail/previous object.  If there is no more room for scrolling the tail/previous object will be focused normally
